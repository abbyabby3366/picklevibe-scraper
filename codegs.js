...
function doPost(e) {
  try {
    ...
    const ss = SpreadsheetApp.openById("1SSgr-xHeVyeTdVX9j283H1Z7eTgmjM0W1BZgb2CN_ao");

    if (!Array.isArray(data)) {
      return ContentService.createTextOutput(JSON.stringify({ status: "error", message: "Data must be an array" }))
    ...
    for (const [sheetName, rows] of Object.entries(groupedData)) {
      if (rows.length > 0) {
        ...
        if (!sheet) {
          ...
          const headers = ["Organization", "Booking ID", "Customer Name", "Phone", "Email", "Start Date & Time", "Duration", "Resources", "Price", "Source", "Status"];
    ...
    return ContentService.createTextOutput(JSON.stringify({ status: "success", count: totalAdded }))
      .setMimeType(ContentService.MimeType.JSON);
